...
def test_post_wizard_storingfood_multiple_wizard_id(token: str, test_case):
    ...
    if response == None:
        pytest.log_objects[__name__].writeToLogFileAsList([str(datetime.datetime.now()), f"Request timed out {attempts} time/s", test_case["wizard_id"], _payload[0]["category"], url])
        assert False

    if not response.status_code in [200, 400, 500]:
        pytest.log_objects[__name__].writeToLogFileAsList([str(datetime.datetime.now()), f"Unknown response status code: { str(response.status_code) }", test_case["wizard_id"], _payload[0]["category"], url])
        assert False
    ...
    try:
        ...
        pytest.log_objects[__name__].writeToLogFileAsList([str(datetime.datetime.now()), f"Exception: {ex}, Malformed data: {str(response.text)}", test_case["wizard_id"], _payload[0]["category"], url])
        assert False
    
    if len(data) <= 0:
        pytest.log_objects[__name__].writeToLogFileAsList([str(datetime.datetime.now()), f"Empty response: {data}", test_case["wizard_id"], _payload[0]["category"], url])
        assert False
    ...
    if isValidOrTrue != True:
        pytest.log_objects[__name__].writeToLogFileAsList([str(datetime.datetime.now()), f"{isValidOrTrue}", test_case["wizard_id"], _payload[0]["category"], url])
        assert False
    
    if response.status_code == 200:
        first_element_in_data = data[0]
        errors = ""
        if "storageZone" in _expected_results:
            errors += get_possible_errors(first_element_in_data, _expected_results, "storageZone")
        if "freezerZone" in _expected_results:
            errors += get_possible_errors(first_element_in_data, _expected_results, "freezerZone")
        if "category" in _expected_results:
            errors += get_possible_errors(first_element_in_data, _expected_results, "category")
        if "storageShelf" in _expected_results:
            errors += get_possible_errors(first_element_in_data, _expected_results, "storageShelf")
        if "freezerShelf" in _expected_results:
            errors += get_possible_errors(first_element_in_data, _expected_results, "freezerShelf")
        if "storageTemperature" in _expected_results:
            errors += get_possible_errors(first_element_in_data, _expected_results, "storageTemperature")
        if "freezerTemperature" in _expected_results:
            errors += get_possible_errors(first_element_in_data, _expected_results, "freezerTemperature")
        if "duration" in _expected_results:
            errors += get_possible_errors(first_element_in_data, _expected_results, "duration")
        if "freezerDuration" in _expected_results:
            errors += get_possible_errors(first_element_in_data, _expected_results, "freezerDuration")
        errors += get_possible_errors(first_element_in_data, _expected_results, "wizardId")
        ...
        pytest.log_objects[__name__].writeToLogFileAsList([str(datetime.datetime.now()), f"{data}", test_case["wizard_id"], _payload[0]["category"], url])
        assert False
    elif response.status_code == 500:
        pytest.log_objects[__name__].writeToLogFileAsList([str(datetime.datetime.now()), f"errorMessage: {data['errorMessage']}, errorId: {data['errorId']}", test_case["wizard_id"], _payload[0]["category"], url])
        assert False
    else:
        pytest.log_objects[__name__].writeToLogFileAsList([str(datetime.datetime.now()), f"Unhandled response with status code: {response.status_code}", test_case["wizard_id"], _payload[0]["category"], url])
        assert False
