...
wizard_ids = getWizardIdsForType("Dishwasher", ReadFileFromSharedDataDirectory("collected_wizards.json"))
dishwasher_dir_path = os.path.join(ROOT_PROJECT_PATH, "ExcelFiles/Dishwasher")
dishwasher_dir = list(map(lambda x: x.lower(), os.listdir(dishwasher_dir_path)))
...
for wizard_id in wizard_ids:
    ...
    file_name = get_item_from_list(unique_name, dishwasher_dir)
    if file_name != None and "default" in file_name:
        file_name = None
    if file_name != None:
        excel_file_pd = pandas.read_excel(os.path.join(dishwasher_dir_path, file_name), sheet_name="Dishwasher_v2_wash")
        ...
        for idx in excel_file_pd.index:
            ...
            row["payload"]["soilLevel"] = str(excel_file_pd["SOIL_LEVEL"][idx])
            row["payload"]["wayOfWash"] = str(excel_file_pd["WAY_OF_WASH"][idx])

            row["expected_response_obj"]["program"] = str(excel_file_pd["PROGRAM"][idx])
            if str(excel_file_pd["PROGRAM_MODE"][idx]) == "nan" or str(excel_file_pd["PROGRAM_MODE"][idx]).strip() == "":
                row["expected_response_obj"]["programMode"] = ""
            else:
                row["expected_response_obj"]["programMode"] = str(excel_file_pd["PROGRAM_MODE"][idx])
            row["expected_response_obj"]["totalRunTime"] = str(excel_file_pd["TOTAL_RUN_TIME"][idx])

            if row["payload"]["type"][0] == "nan" and row["payload"]["soilLevel"] == "nan" and row["payload"]["wayOfWash"] == "nan":
...
def test_post_wizard_dishwasher_wizard_id(token: str, test_case_obj):
    pytest.log_objects[__name__].writeHeaderToLogFileAsList(["time", "error", "wizard_id", "payload", "endpoint"])
    url = f"{pytest.api_base_url}/api/v1/wizard/dishwasher/{test_case_obj['wizard_id']}"
    print("\nTesting " + url)
    ...
    if response.status_code == 200:
        errors = ""
        errors += get_possible_errors(data, test_case_obj["expected_response_obj"], "program")
        errors += get_possible_errors(data, test_case_obj["expected_response_obj"], "programMode")
        errors += get_possible_errors(data, test_case_obj["expected_response_obj"], "totalRunTime")
