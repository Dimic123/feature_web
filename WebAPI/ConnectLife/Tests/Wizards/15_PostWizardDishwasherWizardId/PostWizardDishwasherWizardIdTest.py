        ...
        if any(x["wizard_id"] == wizard_obj["wizard_id"] for x in wizard_ids):
            if not "default" in wizard_obj["wizard_id"]:
                ...
                for config in excel_configuration_list:
                    if config["name"] == "dishwasher_wizard_sheet":
                        dishwasher_wizard_sheet = config["excel_sheet"]

                for test_case in dishwasher_wizard_sheet:
                    if not "default" in wizard_obj["wizard_id"]:
                        test_case["wizard_id"] = wizard_obj["wizard_id"]
                all_test_cases_with_wizard_id += dishwasher_wizard_sheet
...
def test_post_wizard_dishwasher_wizard_id(token: str, test_case_obj):
    ...
    if "PROGRAM" in test_case_obj["outputs"]:
        _expected_results["program"] = test_case_obj["outputs"]["PROGRAM"]
    if "PROGRAM_MODE" in test_case_obj["outputs"]:
        _expected_results["programMode"] = str(test_case_obj["outputs"]["PROGRAM_MODE"]).strip()
    if "OPTION" in test_case_obj["outputs"]:
        _expected_results["option"] = test_case_obj["outputs"]["OPTION"]
    if "TOTAL_RUN_TIME" in test_case_obj["outputs"]:
        _expected_results["totalRunTime"] = test_case_obj["outputs"]["TOTAL_RUN_TIME"]
    ...
    if response.status_code == 200:
        errors = ""
        if "program" in _expected_results:
            errors += get_possible_errors(data, _expected_results, "program")
        if "programMode" in _expected_results:
            errors += get_possible_errors(data, _expected_results, "programMode")
        # if "option" in _expected_results: #### TODO why is option missing from response??
        #     errors += get_possible_errors(data, _expected_results, "option")
        # if "totalRunTime" in _expected_results:
        #     errors += get_possible_errors(data, _expected_results, "totalRunTime")
