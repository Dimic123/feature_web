...
@pytest.mark.prod
def test_get_wizard_all(token: str, params: dict):
    url = "https://api.connectlife.io/api/v1/wizard/all" + ("?includeLogicData=" + str(params["includeLogicData"]) if type(params["includeLogicData"]) == type(True) else "")
    ...
    response = SendRequest("GET", url, headers, payload, 100)
...
def CreateJsonSchemas():
    ...
    success200Schema = CreateJsonSchema(
        "GET response schema for /api/v1/wizard/all/includeLogicCode requests: SUCCESS 200", 
    ...
    err500Schema = CreateJsonSchema(
        "GET response schema for /api/v1/wizard/all/includeLogicCode requests: ERROR 500", 
        "Wizard response schema", 
