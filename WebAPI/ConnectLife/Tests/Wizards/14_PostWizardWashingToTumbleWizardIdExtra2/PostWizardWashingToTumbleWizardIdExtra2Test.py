...
for wizard_id in wizard_ids:
    ...
    if file_name != None and "default" in file_name:
        file_name = None
    if file_name != None:
        excel_file_pd = pandas.read_excel(os.path.join(wm_to_td_dir_path, file_name), sheet_name="Sensitivity")
        ...
        for idx in excel_file_pd.index:
            ...
            row["payload"]["sensitivity"] = str(excel_file_pd["SENSITIVITY"][idx])

            row["expected_response_obj"]["motorSpinSpeed"] = str(excel_file_pd["MOTOR_SPIN_SPEED"][idx])

            if row["payload"]["type"][0] == "nan" and row["payload"]["sensitivity"] == "nan":
...
if all_test_cases == []:
    for wizard_id in wizard_ids:
        if "default" in wizard_id["wizard_id"]:
            row = { "wizard_id": wizard_id["wizard_id"], "payload": {}, "expected_response_obj": {} }
            row["payload"] = { "type": [ "Business" ], "sensitivity": "Low" }
            row["expected_response_obj"] = { "motorSpinSpeed": 1000 }
            all_test_cases.append(row)

@pytest.mark.test_env
@pytest.mark.parametrize("test_case_obj", all_test_cases)
def test_post_wizard_washing_to_tumble_wizard_id_extra2(token: str, test_case_obj):
    pytest.log_objects[__name__].writeHeaderToLogFileAsList(["time", "error", "wizard_id", "payload", "endpoint"])
    url = f"{pytest.api_base_url}/api/v1/wizard/washing-to-tumble/{test_case_obj['wizard_id']}/extra2"
    print("\nTesting " + url)
