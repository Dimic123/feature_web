...
@pytest.mark.prod
def test_post_wizard_storingfood_wizard_id(token: str, params: dict):
    url = "https://api.connectlife.io/api/v1/wizard/storingfood/" + params["wizardid"]
...
def CreateJsonSchemas():
    ...
    success200Schema = CreateJsonSchema(
        "GET response schema for /api/v1/wizard/storingfood/{wizardid} requests: SUCCESS 200", 
    ...
    err500Schema = CreateJsonSchema(
        "GET response schema for /api/v1/wizard/storingfood/{wizardid} requests: ERROR 500", 
        "Wizard response schema", 
