...
from WebAPI.ConnectLife.Tests.Wizards.base_jsonschema_error_respose import base_jsonschema_error_response

import pytest

@pytest.mark.prod
def test_post_wizard_storingfood_multiple_wizard_id(token: str, params: dict):
    url = "https://api.connectlife.io/api/v1/wizard/storingfood/multiple/" + params["wizardid"]
    filePath = os.path.realpath(__file__)
    
    print("\nTesting " + url)
    [successSchema, errorSchema, err400Schema] = CreateJsonSchemas()
    ...
    if (response.status_code == 200):
        ...
        AssertValidateJson(data, err400Schema)

def CreateJsonSchemas():
    filePath = os.path.realpath(__file__)
    success200Schema = CreateJsonSchema(
        "GET response schema for /api/v1/wizard/storingfood/multiple/{wizardid} requests: SUCCESS 200", 
    ...
    err500Schema = CreateJsonSchema(
        "GET response schema for /api/v1/wizard/storingfood/multiple/{wizardid} requests: ERROR 500", 
        "Wizard response schema", 
    ...
    err400Schema = CreateJsonSchema(
        "GET response schema for /api/v1/wizard/storingfood/multiple/{wizardid} requests: ERROR 400", 
        "Wizard response schema", 
        {
            "type": "string",
            "title": "string",
    ...
    WriteDataToJsonFileInCurrentDirectory("_jsonschema_400", filePath, err400Schema)
    
    return [success200Schema, err500Schema, err400Schema]
