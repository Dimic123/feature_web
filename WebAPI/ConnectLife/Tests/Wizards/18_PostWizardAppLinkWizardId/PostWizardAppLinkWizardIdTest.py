...
app_link_dir_path = os.path.join(ROOT_PROJECT_PATH, "ExcelFiles/AppLink")
app_link_dir = os.listdir(app_link_dir_path)
...
for wizard_id in wizard_ids:
    ...
    file_name = get_item_from_list(unique_name, app_link_dir)
    # if file_name != None and "default" in file_name:
    #     file_name = None
    if file_name != None:
        excel_file_pd = pandas.read_excel(os.path.join(app_link_dir_path, file_name))
        ...
        for idx in excel_file_pd.index:
            ...
            row["payload"]["country"] = str(excel_file_pd["COUNTRY"][idx])
            row["payload"]["linkType"] = str(excel_file_pd["LINK_TYPE"][idx])

            if not isNaN(excel_file_pd["URL"][idx]):
                row["expected_response_obj"]["url"] = str(excel_file_pd["URL"][idx])

            if not isNaN(excel_file_pd["NAME"][idx]):
                row["expected_response_obj"]["name"] = str(excel_file_pd["NAME"][idx])

            if not isNaN(excel_file_pd["IMAGE_URL"][idx]):
                row["expected_response_obj"]["imageUrl"] = str(excel_file_pd["IMAGE_URL"][idx])

            if row["payload"]["country"] == "nan" and row["payload"]["linkType"] == "nan":
...
def test_post_wizard_app_link_wizard_id(token: str, test_case_obj):
    ...
    if response.status_code == 200:
        errors = ""
        errors += get_possible_errors(data, test_case_obj["expected_response_obj"], "url")
        errors += get_possible_errors(data, test_case_obj["expected_response_obj"], "name")
        errors += get_possible_errors(data, test_case_obj["expected_response_obj"], "imageUrl")
