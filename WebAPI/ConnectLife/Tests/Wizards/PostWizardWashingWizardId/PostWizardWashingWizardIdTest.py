...
@pytest.mark.prod
def test_post_wizard_washing_wizard_id(token: str, params: dict):
    url = "https://api.connectlife.io/api/v1/wizard/washing/" + params["wizardid"]
...
def CreateJsonSchemas():
    ...
    success200Schema = CreateJsonSchema(
        "GET response schema for /api/v1/wizard/washing/{wizardid} requests: SUCCESS 200", 
    ...
    err500Schema = CreateJsonSchema(
        "GET response schema for /api/v1/wizard/washing/{wizardid} requests: ERROR 500", 
        "Wizard response schema", 
