...
for wizard_id in wizard_ids:
    ...
    if file_name != None and "default" in file_name:
        file_name = None
    if file_name != None:
        excel_file_pd = pandas.read_excel(os.path.join(washing_dir_path, file_name), sheet_name="Stains")
        ...
        for idx in excel_file_pd.index:
            ...
            row["payload"]["color"] = excel_file_pd["COLOR"][idx]
            row["payload"]["stain"] = excel_file_pd["STAIN"][idx]
            ...
            if isNaN(row["payload"]["stain"]) and isNaN(row["payload"]["color"]) and isNaN(row["payload"]["dirtiness"]):
...
def test_post_wizard_washing_wizard_id_extra1(token: str, test_case_obj):
    pytest.log_objects[__name__].writeHeaderToLogFileAsList(["time", "error", "wizard_id", "payload", "endpoint"])
    url = f"{pytest.api_base_url}/api/v1/wizard/washing/{test_case_obj['wizard_id']}/extra1"
    print("\nTesting " + url)
