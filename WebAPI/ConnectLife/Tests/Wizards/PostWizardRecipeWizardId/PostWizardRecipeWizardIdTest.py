...
@pytest.mark.prod
def test_post_wizard_recipe_wizard_id(token: str, params: dict):
    url = "https://api.connectlife.io/api/v1/wizard/recipe/" + params["wizardid"]
    ...
    response = SendRequest("POST", url, headers, payload, 100)
...
def CreateJsonSchemas():
    ...
    err500Schema = CreateJsonSchema(
        "GET response schema for /api/v1/wizard/recipe/{wizardid} requests: ERROR 500", 
        "Wizard response schema", 
