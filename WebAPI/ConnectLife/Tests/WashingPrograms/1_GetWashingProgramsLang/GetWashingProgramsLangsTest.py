...
def test_get_washing_programs_lang(token: str, lang):
    ...
    url = f"{pytest.api_base_url}/api/v1/washing-programs/{lang}"
    print("\nTesting " + url)
    ...
    if response.status_code == 200:
        global washing_programs_ids_langs
        for i in data:
            washing_programs_ids_langs.append({
                "id": i["id"],
                "lang": lang
            })
        SaveToSharedDataDirectory("washing_programs_ids_langs.json", washing_programs_ids_langs)
    elif response.status_code == 500:
