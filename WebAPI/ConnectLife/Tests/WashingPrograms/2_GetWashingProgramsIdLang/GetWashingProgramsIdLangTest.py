...
def test_get_washing_programs_id_lang(token: str, id_lang_pair):
    ...
    url = f"{pytest.api_base_url}/api/v1/washing-programs/{id_lang_pair['id']}/{id_lang_pair['lang']}"
    print("\nTesting " + url)
    ...
    if response == None:
        pytest.log_objects[__name__].writeToLogFileAsList([str(datetime.datetime.now()), f"Request timed out {attempts} time/s", id_lang_pair['id'], id_lang_pair['lang'], url])
        assert False

    if not response.status_code in [200, 500]:
        pytest.log_objects[__name__].writeToLogFileAsList([str(datetime.datetime.now()), f"Unknown response status code: { str(response.status_code) }", id_lang_pair['id'], id_lang_pair['lang'], url])
        assert False
    ...
    try:
        ...
        pytest.log_objects[__name__].writeToLogFileAsList([str(datetime.datetime.now()), f"Exception: {ex}, Malformed data: {str(response.text)}", id_lang_pair['id'], id_lang_pair['lang'], url])
        assert False
    
    if len(data) <= 0:
        pytest.log_objects[__name__].writeToLogFileAsList([str(datetime.datetime.now()), f"Empty response: {data}", id_lang_pair['id'], id_lang_pair['lang'], url])
        assert False
    ...
    if isValidOrTrue != True:
        pytest.log_objects[__name__].writeToLogFileAsList([str(datetime.datetime.now()), f"{isValidOrTrue}", id_lang_pair['id'], id_lang_pair['lang'], url])
        assert False
    
    if response.status_code == 200:
        pass
    elif response.status_code == 500:
        pytest.log_objects[__name__].writeToLogFileAsList([str(datetime.datetime.now()), f"errorMessage: {data['errorMessage']}, errorId: {data['errorId']}", id_lang_pair['id'], id_lang_pair['lang'], url])
        assert False
    else:
        pytest.log_objects[__name__].writeToLogFileAsList([str(datetime.datetime.now()), f"Unhandled response with status code: {response.status_code}", id_lang_pair['id'], id_lang_pair['lang'], url])
        assert False
