...
def test_get_appliances_profile_warranty_ids_retailers(token: str, id):
    pytest.log_objects[__name__].writeHeaderToLogFileAsList(["time", "error", "id", "endpoint"])
    url = f"{pytest.api_base_url}/api/v1/appliance/profile/warranty/{id}/retailers"
    print("\nTesting " + url)
...
def CreateJsonSchemas():
    ...
    success_200_schema = CreateJsonSchema(
        "GET /api/v1/appliance/profile/warranty/{id}: SUCCESS 200", 
        "Gets appliance warranty information.",
        [
            {
                "code": "string",
    ...
    error_400_schema = CreateJsonSchema(
        "GET /api/v1/appliance/profile/warranty/{id}: ERROR 400", 
        "Error 400", 
