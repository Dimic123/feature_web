...
def test_get_appliances_ids(token: str, id):
    ...
    url = f"{pytest.api_base_url}/api/v1/appliance/{id}"
    print("\nTesting " + url)
...
def CreateJsonSchemas():
    ...
    success_200_schema = CreateJsonSchema(
        "GET /api/v1/appliance/{id}: SUCCESS 200", 
        "Gets current property values for selected appliances.",
        [
            {
                "id": "string",
                "name": "string",
                "type": "string",
                "status": "string",
                "properties+allowAdditionalProperties": {
