...
@pytest.mark.prod
def test_get_recipes_ingredients_lang(token: str, params: dict):
    url = "https://api.connectlife.io/api/v1/recipes/ingredients/" + params["lang"]
...
def CreateJsonSchemas():
    ...
    success200Schema = CreateJsonSchema(
        "GET response schema for /api/v1/recipes/ingredients/{lang} requests: SUCCESS 200", 
    ...
    err500Schema = CreateJsonSchema(
        "GET response schema for /api/v1/recipes/ingredients/{lang} requests: ERROR 500", 
        "Recipe response schema", 
